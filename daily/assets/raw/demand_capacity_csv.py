...
depends:
    - raw.demand_capacity_csv_content

description: |
    Parsed demand+capacity CSV rows decoded from
    raw.demand_capacity_csv_content. Pure transform — no HTTP. One row per
    (content_hash, timestamp, series). Empty while upstream feed is 404'd.
...
def materialize() -> pd.DataFrame:
    with duckdb.connect(str(DB_PATH), read_only=True) as conn:
        content = conn.sql(
            "SELECT content_hash, content_text FROM raw.demand_capacity_csv_content"
        ).df()
