""" @bruin
name: raw.demand_capacity_powerbi
...
depends:
    - raw.demand_capacity_powerbi_content

description: |
    Parsed PowerBI points for the demand+capacity feed, decoded from
    raw.demand_capacity_powerbi_content. Pure transform — no HTTP. One row
    per (response_hash, timestamp, series).
...
def materialize() -> pd.DataFrame:
    with duckdb.connect(str(DB_PATH), read_only=True) as conn:
        content = conn.sql(
            ...
            "FROM raw.demand_capacity_powerbi_content"
        ).df()
