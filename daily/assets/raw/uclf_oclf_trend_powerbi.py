""" @bruin
name: raw.uclf_oclf_trend_powerbi
...
depends:
    - raw.uclf_oclf_trend_powerbi_content

description: |
    Parsed PowerBI points for the Hourly UCLF+OCLF Trend feed, decoded from
    raw.uclf_oclf_trend_powerbi_content. Pure transform — no HTTP. Series:
    'Hourly UCLF+OCLF' (combined, MW). One row per (response_hash, timestamp,
    series).
...
def materialize() -> pd.DataFrame:
    with duckdb.connect(str(DB_PATH), read_only=True) as conn:
        content = conn.sql(
            ...
            "FROM raw.uclf_oclf_trend_powerbi_content"
        ).df()
