...
depends:
    - raw.supply_build_up_csv_content

description: |
    Parsed CSV measurements, decoded from raw.supply_build_up_csv_content.
    Pure transform — no HTTP. One row per (content_hash, timestamp, series).
...
def materialize() -> pd.DataFrame:
    with duckdb.connect(str(DB_PATH), read_only=True) as conn:
        content = conn.sql(
            "SELECT content_hash, content_text FROM raw.supply_build_up_csv_content"
        ).df()
