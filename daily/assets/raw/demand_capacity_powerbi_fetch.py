""" @bruin
name: raw.demand_capacity_powerbi_fetch
...
description: |
    Fresh fetch of the PowerBI report embedded in the demand-side
    System hourly demand and available capacity page. Holds only THIS
    run's scrape; downstream SQL splits into the _scrapes log and the
    deduped _content store.

    Useful even though the CSV download is broken, because the PowerBI
    iframe is still being updated and gives ~last-week-ish coverage of
    available capacity. Depends on the CSV fetch only to serialise
